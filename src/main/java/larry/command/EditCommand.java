...
public class EditCommand extends Command {
    public static final String INVALID_EVENT_ORDER_MESSAGE =
    ...
    public void execute(TaskList tasks, Ui ui, Storage storage) throws LarryException {
        if (taskIndex < 0 || taskIndex >= tasks.size()) {
            throw new LarryException("ERROR: There is no task at index "
                    + (taskIndex + 1) + " to edit.");
        }
    ...
    private LarryException incompatibleField(Task task) {
        return new LarryException("ERROR: You cannot edit " + field.getMarker()
